        ...
        if(tradingEngine.isWithinTradingHours()) {
            ...
            if (choice == 1) {
                ...
                if (choice == 1) {
                    ...
                    buyStock = findStockBySymbol(stocks, buyStockSymbol);
                    if (buyStock != null) {
                        Order buyOrder = new Order(buyStock, Order.Type.BUY, buyQuantity, 0.0, buyExpectedPrice, 0.0);
                    ...
                    sellStock = findStockBySymbol(stocks, sellStockSymbol);
                    if (sellStock != null) {
                        Order sellOrder = new Order(sellStock, Order.Type.SELL, sellQuantity, 0.0, 0.0, sellExpectedPrice);
                ...
                System.out.println("Cancel pending orders based on: \n1. Longest time \n2. Highest amount of price");
                choice = scanner.nextInt();
                if(choice == 1){
                    //cancel pending order based on longest time
                }else if(choice == 2){
                    //cancel pending order based on highest amount
                }
            }else if(choice == 4){
